...
from functools import \
    partial  # needed adding callback functions with correct variables in for loops
...
class Ledger:
    def __init__(self, master, title, row_num, column_num, *args, **kwargs):
        ...
        self.m_sel_ind = [
            0
        ] * 2  # for storing a group select ('shift' key down) of multiple transactions. This stores transaction # NOT sql key
    ...
    def del_sel_trans(self):
        ...
        gui_helper.gui_print(
            self.frame,
            self.prompt,
            "Deleting the transactions with the following sql keys: ",
        )
    ...
    def change_transaction_category(self, index, *args):
        gui_helper.gui_print(
            ...
            + str(index)
            + " to category: "
            + self.clicked_category[index].get(),
        )
        category_id = categories_helper.category_name_to_id(
            self.clicked_category[index].get()
        )
    ...
    def createStatementTable(self):
        ...
        delete_statement = tk.Button(
            self.fr_action, text="Delete Statement", command=self.delete_statement
        )
        delete_statement.grid(row=0, column=0, padx=10, pady=10)

        # place button for saving statement
        save_statement = tk.Button(
            self.fr_action, text="Update Statement", command=self.save_statement
        )
        ...
        del_sel = tk.Button(
            self.fr_sel_action,
            text="Delete Selected Transactions",
            command=self.del_sel_trans,
        )
    ...
    def init_ledger_table(self):
        ...
        self.frame_canvas.grid(
            row=1, column=0, pady=(5, 0), padx=10, sticky="nw", columnspan=5
        )
    ...
    def pop_ledger_top_row(self, *args):
        ...
        dropdown = tk.OptionMenu(
            self.frame, variable, *sort_options, command=change_sort
        )
    ...
    def pop_ledger_data(self):
        ...
        labels = [
            [tk.Label() for j in range(columns)] for i in range(rows)
        ]  # populate 2D array of labels
        ...
        for i in range(0, rows):
            ...
            def trans_callback(i, sql_key, button_press):
                ...
                if shift_key_down:
                    ...
                    self.m_sel_ind.append(
                        i
                    )  # append the NUMBER of the transaction (note SQL key)
            ...
            labels[i][0].bind(
                "<Double-Button-1>", partial(trans_callback, i, string_dict["sql_key"])
            )  # lambda 'x': needed because ButtonPress state is called back. Ex: <ButtonPress event state=Mod1 num=1 x=16 y=18>

            # date, description, and amount
            labels[i][1] = tk.Label(self.frame_data, text=string_dict["date"])  # date
            labels[i][2] = tk.Label(
                self.frame_data, text=string_dict["description"]
            )  # description
            labels[i][3] = tk.Label(
                self.frame_data, text=string_dict["amount"]
            )  # amount
            ...
            if string_dict["category"] == 0:
                self.clicked_category[i] = tk.StringVar(
                    self.frame_data
                )  # datatype of menu text
                self.clicked_category[i].set(
                    "Please select a category"
                )  # initial menu text
                self.clicked_category[i].trace(
                    "w", partial(self.change_transaction_category, i)
                )
                labels[i][4] = tk.OptionMenu(
                    self.frame_data,
                    self.clicked_category[i],
                    # TODO
            ...
            # if the transaction already has a category assigned
                ...
                labels[i][4] = tk.Label(
                    self.frame_data,
                    text=categories_helper.category_id_to_name(string_dict["category"]),
                )
    ...
    def update_ledger_sizing(self, labels):
        ...
        first_rows_height = sum(
            [
                labels[i][0].winfo_height()
                for i in range(0, min(len(self.transactions), 15))
            ]
        )  # this number (15) sets number of rows displayed
        self.frame_canvas.config(
            width=first_columns_width + self.vsb.winfo_width(), height=first_rows_height
        )
