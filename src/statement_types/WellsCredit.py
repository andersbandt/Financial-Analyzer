...
class WellsCredit(Statement.Statement):
    def __init__(self, master, account_id, year, month, file, row_num, column_num, *args, **kwargs):
        ...
        self.title = "Wells Credit: " + str(self.account_id) + ":" + str(self.year) + "-" + str(self.month)
    ...
    def load_statement_data(self):
        transactions = []
        gui_helper.gui_print(self.frame, self.prompt, "Extracting raw Wells Credit statement at: ", self.filepath)
        try:
