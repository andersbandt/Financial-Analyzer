...
class Venmo(Statement.Statement):
    def load_statement_data(self):
        ...
        gui_helper.gui_print(
            self.frame,
            self.prompt,
            "Extracting raw Venmo statement at: ",
            self.filepath,
        )
        try:
            with open(self.filepath) as f:
                ...
                for line in csv_reader:
                    ...
                    if line[14] == "Venmo balance":
                        ...
                        amount = (
                            line[8][0] + line[8][3:]
                        )  # first grab the sign (always negative?) and then add the value without a dollar sign
                        description = (
                            line[7] + "-" + line[5]
                        )  # person receiving it + description
                        transactions.append(
                            Transaction.Transaction(
                                date, self.account_id, None, amount, description
                            )
