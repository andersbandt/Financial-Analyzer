...
import datetime
from extract import ExtractedDocument, Transaction, Payment
        ...
        for page in page_copy.values(): # we will be removing items from the dict, so we need a copy
            ...
            for word in page:
                if re.match(re.compile(r'^\d{2}/\d{2}/\d{4}$'), word):
                    ...
                    page.remove(word) # remove the date from the list so we don't find it again on the next .index() lookup
        return transactions
