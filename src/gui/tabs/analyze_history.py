...
class tabSpendingHistory:
    def __init__(self, master):
        ...
        self.canvas = tk.Canvas(
            self.frameX, width=1200, height=800
        )  # for some reason increasing the height breaks the scrollbar?
        ...
        self.canvas.configure(
            yscrollcommand=scroll.set, scrollregion=self.canvas.bbox("all")
        )
    ...
    def init_date_account_selection(self):
        # print frame title
        tk.Label(
            self.fr_date_select, text="Select Date Range", font=("Arial", 16)
        ...
        start_analyzing = ttk.Button(
            self.fr_date_select,
            text="Recall Data",
            command=lambda: self.analyze_history(
                cal_start.get_date(), cal_end.get_date(), self.accounts
            ),
        )
        start_analyzing.grid(
    ...
    def analyze_history(self, date_start, date_end, accounts):
        ...
        if len(accounts) == 0:
            gui_helper.alert_user(
                "Error with recalling data", "No accounts selected.", "error"
            )
        ...
        if not date_helper.date_order(formatted_start, formatted_end):
            gui_helper.alert_user(
                "Error: can't analyze spending history", "Invalid date range", "error"
            )

        ### recall transaction data and display it on the Frame
        # get transactions
        self.transactions = analyzer_helper.recall_transaction_data(
            formatted_start, formatted_end, accounts
        )
    ...
    def show_graphical_processing_gui(self, frame):
        ...
        ttk.Button(
            frame, text="Show Pie Chart", command=lambda: self.show_pie_chart()
        ).grid(row=1, column=0, padx=5)

        # set up button to show a summary over time of the transactions recalled
        ttk.Button(
            frame,
            text="Show Pie Chart (top level)",
            command=lambda: self.show_top_pie_chart(),
        ).grid(row=1, column=1, padx=5)

        # set up button to show pie chart of all categories and expenses
        ttk.Button(
            frame, text="Show Bar Chart", command=lambda: self.create_bar_chart()
        ).grid(row=1, column=2, padx=5)

        # set up button to show a summary over time of the transactions recalled
        ttk.Button(
            frame,
            text="Show Sums over Time",
            command=lambda: self.create_summation_vs_time(),
        ).grid(row=1, column=3, padx=5)

        # set up button to show a summary over time of the transactions recalled
        ttk.Button(
            frame,
            text="Show Budget Diff",
            command=lambda: self.show_budget_diff_chart(),
    ...
    def check_data_load_status(self):
        if not self.data_loaded:
            gui_helper.alert_user(
                "WARNING", "No ledger data is loaded in yet.", "warning"
            )
        return
